<div class="page-inner">
	<div class="page-header">
		<h4 class="page-title">Input KG <input type="hidden" id="kgannya"> </h4>
...
$(document).ready(function () {
	...
	create_tabel(idwo,id[2],id[0],id[4]);
	$.ajax({
			type: "POST",
...
function savekgonhidden(){
	var id = '<?php echo $_GET['id'] ?>'.split(';');
	var idso = id[2];
	$.ajax({
		type: "POST",
		url: "pages/pemeteraian/checkvalue.php",
		data: {idso : idso},
		dataType: "text",
		success: function (response) {
			response = JSON.parse(response);
			$("#kgannya").val(response);
		}
	});
}
...
function checkvalue(el){
	...
	var value = $(el).val();
	index = kg.indexOf(value);
	// console.log(index);
	if(index == -1){
		$(el).css('background-color', 'red');
		$(el).css('color', 'white');
		$(el).css('border', '1px solid red');
		$(el).after('<div> KG"an Tidak Di Temukan! </div></td>');
		// $(el).closest('div').tooltip('show');
	}else if(index > -1){
		// remove from kg
		kg.splice(index,1);
	...
	function create_tabel(idwo,idso,idjw,idgrey){
		$.ajax({
			type: 'POST',
			url: 'pages/pemeteraian/fetch.php',
			data: { wo: idwo ,so:idso,jw:idjw,grey:idgrey},
			dataType :"text",
			success: function(response) {
				var html = '';
				...
				for (var i = 0; i < datanya.length; i++) {
					...
					var other = '<?php echo $_GET['other'] ?>'.split(';');
					var rollfix=0;
					// console.log(other[6]);
					// console.log(other[5]);
					if(other[6] == 'manual'){
					...
					for (var j = 0; j < rollfix; j++) {
						html += '<tr><td>'+no+'</td>';
						html += '<td contenteditable="false" class=""><input type="number" class="kg" onchange="javascript:checkvalue(this);" style="width:95px;" />';
